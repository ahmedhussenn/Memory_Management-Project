...
int allocate_frame(struct Frame_Info **ptr_frame_info)
{
	*ptr_frame_info = LIST_FIRST(&free_frame_list);
	...
	if (*ptr_frame_info == NULL)
	{
		...
		{
			struct WorkingSetElement * ptr = LIST_LAST(&(curenv->SecondList));
			uint32 page_permissions = pt_get_page_permissions(curenv, ptr->virtual_address);
			if(page_permissions & PERM_MODIFIED)
			{
				uint32 * ptr_page_table = NULL;
				get_page_table(curenv->env_page_directory, (void*)ptr->virtual_address, &ptr_page_table);
				struct Frame_Info *ptr_frame = get_frame_info(curenv->env_page_directory, (void*)ptr->virtual_address, ptr_page_table);
				int ret = pf_update_env_page(curenv, (void*)ptr->virtual_address, ptr_frame);

			}
			unmap_frame(curenv->env_page_directory, ptr->virtual_address);
			LIST_REMOVE(&(curenv->SecondList), ptr);
		    pt_set_page_permissions(curenv, ptr->virtual_address, 0, PERM_PRESENT);
			LIST_INSERT_HEAD(&(curenv->PageWorkingSetList), ptr);

		}
		else if (LIST_SIZE(curenv->SecondList) == 0)
		{
			struct WorkingSetElement * ptr = LIST_LAST(curenv.ActiveList);
			uint32 page_permissions = pt_get_page_permissions(curenv, ptr->virtual_address);
			if(page_permissions & PERM_MODIFIED)
			{
							uint32 * ptr_page_table = NULL;
							get_page_table(curenv->env_page_directory, ptr->virtual_address, &ptr_page_table);
							struct Frame_Info *ptr_frame = get_frame_info(curenv->env_page_directory, ptr->virtual_address, ptr_page_table);
							int ret = pf_update_env_page(curenv, ptr->virtual_address, ptr_frame);

						}
						unmap_frame(curenv->env_page_directory, ptr->virtual_address);
						LIST_REMOVE(&(curenv->ActiveList), ptr);
					    pt_set_page_permissions(curenv, ptr->virtual_address, 0, PERM_PRESENT);
						LIST_INSERT_HEAD(&(curenv->PageWorkingSetList), ptr);
...
void allocateMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	//cprintf("size = %d " , size/PAGE_SIZE);
	while(number_page > 0)
	{
		int re = pf_add_empty_env_page(e , add , 0) ;
		if(re == 0)
		{
			add+=PAGE_SIZE ;
			number_page--;
...
// [2] freeMem
//LIST_FOREACH(looper, &(e->SecondList)) // delete from WS
//								  {
//							if(looper->virtual_address==v_add)
//									    {
//										  struct WorkingSetElement * from_second_to_active=looper;
//											LIST_REMOVE(&(e->SecondList), looper);
//											unmap_frame(e->env_page_directory,(void*)looper->virtual_address);
//											break;
//									     }
//									}
//	struct WorkingSetElement * from_second_to_active=LIST_FIRST(&(e->SecondList));
//										LIST_REMOVE(&(e->SecondList), from_second_to_active) ;
//										pt_set_page_permissions( e ,from_second_to_active->virtual_address,PERM_PRESENT,0 );
//										LIST_INSERT_TAIL(&(e->ActiveList) , from_second_to_active) ;
void freeMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	/*		uint32 v_add2 = virtual_address ;
	//		uint32 v_addf  ;
		//		int num2 = size/PAGE_SIZE ;
		//		int numf = num ;
			v_addf = virtual_address +(size) ;*/
